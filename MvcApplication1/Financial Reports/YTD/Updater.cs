...
namespace MvcApplication1.Financial_Reports.YTD
{
    public class Updater
    {
        ...
        public Updater(string _fiscal_year, string _fiscal_month)
        {
            fiscal_month = _fiscal_month;
            fiscal_year = _fiscal_year;
        }
        ...
        public void RefreshData()
        {
            ...
            //{
                ...
                Log.Append("    Retrieving required information...");//d " + (i + 1).ToString() + "...");
        ...
        private void _MARSHAL_KILL_EXCEL(string path)
        {
            ...
            {
                Log.Append("    Error disposing EXCEL files");
            }
            excel.Quit();
                ...
                if (!(WORKSHEET_ARRAY[i, 1] == "Name") && !(WORKSHEET_ARRAY[i, 1] == "0"))
                {
                    ...
                    else if (WORKSHEET_ARRAY[i, 1].Contains("TOTAL NET INCOME")) //end of document
                    {
                        ...
                        //{
                            ...
                            if (plant != "Markham") row = row + 500; //END OF FILE FOR NON-MARKHAM
                        //}
                        //else
                        if (plant == "Markham")
                        {
...
namespace YTD_Updater
{
    public class Updater
    {
        ...
        //public void Update(int plantID)
        {
            ...
            //Log.Append("    =================================================================");
            //Log.Append("    Analyzing for period: " + fiscalPeriod + ", 20" +(fiscalPeriod >= 10 ? fiscalYear - 1 : fiscalYear).ToString());
                ...
                foreach (Group group in category.groupList)
                {
                    ...
                    if (account_name.Length > 0)
                    {
                        ...
                        //for (int fiscalYear = 12; fiscalYear < 18; fiscalYear++)
                        {
                            ...
                            {
                                Log.Append("    Plant: " + plant.plantID.ToString() + " for period: " + fiscalPeriod + ", " + (fiscalPeriod >= 10 ? fiscalYear - 1 : fiscalYear));
                                //Log.Append("           " + account_name + " :US: " + plant.actualThisYearList[fiscalPeriod].GetAmount("US"));
                                Log.Append("           " + account_name + " :CA: " + plant.actualThisYearList[fiscalPeriod].GetAmount("CA"));
                                //Log.Append("           " + account_name + " :CP: " + plant.actualThisYearList[fiscalPeriod].GetAmount("CP"));
                                Log.Append("           " + "AGG_TOTAL = " + Aggregate_Acc_Totals[account_name]);
                            }
                            fiscalPeriod++;
                        }
                    }
            ...
            {
                //Log.Append("    Checking account name: " + account.Key.ToString() + "___ with aggregate sum of: " + account.Value.ToString());
